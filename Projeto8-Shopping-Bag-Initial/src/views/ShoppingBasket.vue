              ...
              <div v-for="(product, index) in productsInBag" :key="index" class="item">
                ...
                <div class="price">
                  <span class="quantity-area">
                    <button :disabled="product.quantify <= 1" @click="product.quantify--">-</button>
                    <span class="quantity">{{ product.quantify }}</span>
                    <button @click="product.quantify++">+</button>
                  </span>
                  <span class="amount">US$ {{ (product.price * product.quantify).toFixed(2)}} </span>
                </div>
              </div>
